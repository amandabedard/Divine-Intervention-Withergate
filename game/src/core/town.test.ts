...
describe('store and tavern', () => {
  it('buys and sells at the posted prices', () => {
    const state = start();
    const ctx = mk(state);
    state.town.resources.gold = 20;
    expect(buyPrice(ctx, 'wood', 3)).toBe(9);
    expect(buy(ctx, 'wood', 3)).toBeNull();
    expect(state.town.resources.gold).toBe(11);
    expect(state.town.resources.wood).toBe(23);
    expect(sell(ctx, 'wood', 10)).toBeNull();
    expect(state.town.resources.gold).toBe(26);
    expect(buy(ctx, 'ore', 100)).toMatch(/Not enough gold/);
  });
