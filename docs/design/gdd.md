...
- Towns: Aboridge (seat of the king; built as three placeholder maps, Lower Aboridge, Upper Aboridge and the throne room), Dilsdurf, Heathel, Mukrige, Scottsburg (identities TBD by Amanda), plus Withergate (yours).

## 2. The player character

### 2.0 The opening (Amanda's script, built)
Prologue pages → character creation → the fall into the glade (`opening`) → forage → a thief steps out of the treeline (the tutorial fight, `opening/first_fight`) → Aldric rides in with two knights and the king's proposal: Withergate is yours, in the hope that divine intervention slows the corruption that has just taken the queen (`the_proposal`, with Amanda's choices and their friendship effects) → the tour of Withergate (`the_tour`) → Aldric waits at your desk; the talk there asks what profession you want first, and only "I was hoping you'd be interested in joining me" makes him recruitable later (`aldric_quarters_talk`) → he travels with you as a **guest** to Aboridge → you walk up through Lower and Upper Aboridge to the castle → the audience (`king_audience`): Aldric presents you and is sent back to the barracks (`unguest`), the king presents Jasper (a former prisoner turned miner) and Gemma (a country logger) and tells you to get to know them and recruit the one you choose → the tutorial recruitment (their recruit topic opens with the audience and needs no roll) → the king's thanks (`the_kings_thanks`, started by act 0's `choose` stage): a loose explanation of how dialog works, thanks for your service, hopes for the two settlements. Act 0 ends there; act 1 starts with the walk home (decided L2; assumptions in M1–M5).
...
**The title is earned, not chosen (decided).** Choices, quests, and deeds award domain points on the five axes throughout the game, based on what you actually do. Nothing is locked in. At the end, the game generates what you are the Deity *of* from your accumulated stats (built: `content/ascension.yaml` holds the words per axis; §13). **It is a secret (decided F5):** no screen, toast, or line hints at the leanings before the ending, so `{domain}` in dialog and the `domain_lean` condition are for the ending and for characters reacting in-character, never for telling the player their score. Content reads the leaning through `domain_lean` and thresholds through `domain_points_min`.
...
The world is **Duluma** (decided), made by Aluma, Deity of Light, and Duskus, Deity of Darkness, whose quarrel is day and night; the lesser deities shaped the region and, after both took human form among mortals, left a scattering of half-deities. The player is one: thought full-blooded, cast down at maturity for having no domain by birthright. The prologue in `content/intro.yaml` tells this before character creation.
...
### 5.3 Corruption
A world value 0–10 (proposed) that rises on a schedule slowed or reversed by story progress. Effects: node weights near the frontier (more monsters, corrupted variants, corrupted gather nodes), incursion events at Withergate (defend it; Barracks and Warriors help), corrupted checkpoints. **Special corruption maps must be cleared; otherwise it starts killing non-recruited NPCs at random (decided).** At risk: any character who is not recruited and not marked `protected: true` on their sheet (the king and story figures are protected). Recruiting someone protects them. **Cadence (decided G3):** a full week (5 days) without clearing corruption brings the warning: "You can feel the corruption leaking into the mortal plane. Better do something about it before something terrible happens." If a corruption dungeon has not been started within the next 3 days, someone is taken, and the messenger reports it: "It appears we lost xyz in H town to the corruption last night…" (decided F4). **Built (Phase 8):** a *corruption expedition* is any region that crosses corrupted ground. Days spent on one do not count as idle; starting one within the three days satisfies the warning; reaching its last checkpoint clears the corruption (the clock starts over, a morning notice says so); turning back or dying resumes the clock with a fresh three-day grace (decided H1). Whoever is taken is picked at random among characters who are not residents and not `protected: true`; they are gone (resurrection is a late-game mechanic driven by a character not yet written, decided L1) and the messenger's line names them and their town, after a purple flash across the screen: "The corruption has reached Duluma…" (decided K3). Left alone, the corruption also rises by one every ten idle days (`progression.yaml → corruption.rise_every_days`; the frontier's checkpoints lower it with `corruption: -1` effects), and from level 6 corrupted things may come at Withergate's walls in the night: the Barracks' `incursion_defense` plus one per Warrior resident against the excess corruption; hold and gain a little faith, fail and the stores lose 15% of each resource. Texts and numbers live in `progression.yaml → corruption`.
...
### 8.5 Party
Up to 2 companions per expedition, chosen from residents, plus **guests**: characters a story effect (`guest: aldric`) attaches to you without their living in Withergate; they are preselected in the planner and stay until `unguest`. Anyone in your party stands nowhere else while you travel (decided J2). Willingness depends on tier and energy. Companions appear in battle and cannot be targeted by enemies (decided). All provide passives; some also have one active battle ability the player triggers (decided). Energy: a max per character; −1 per node, −2 at night or in corruption; at 0 they withdraw and walk home safely (proposed), and you lose their benefits for the rest of the trip. At home, companion energy recovers over days rather than instantly (proposed +2 per day).
...
### 10.2 The node map
Slay-the-Spire style: columns of 2–4 nodes with edges; you pick a path left to right. Each node costs one phase. A **segment** is three nodes and a **checkpoint**, so a segment is a day; town routes have one segment per day and end at the town, explorations 1–3 segments (decided). Only the next column's node types are known; the Library, an Academic and the Wayfinding power each reveal one more. Checkpoints are never shown in advance (decided J3): every node's type is fixed when the map is generated, so a future explorer skill can reveal them, but what waits at a checkpoint (a camp, a caravan of explorers, a mini town, a boss) is found out by arriving. At a checkpoint you take stock: press on, send the haul home by caravan, or head home. The same map holds the way back: a route ending in a town lists the reverse journey from that town's gate.
...
- **Journal UI (built):** `J` in the world or Journal at the desk lists active quests with the current objective and the notes of finished stages, then finished and failed quests. A quest whose giver the corruption took is greyed and waits rather than failing, since characters can be brought back late in the game by a character not yet written (decided K2 and L1).
- **Main story skeleton (placeholder, Amanda owns it):** Act 0 the fall and the messenger → Act 1 founding Withergate, meeting the five towns, domain lock-in → Act 2 the discord between the cities while incursions escalate → Act 3 the source of the corruption → Ascension as the Deity of ___, with an epilogue shaped by choices. The chain is scaffolded in `content/quests/main_act1..3.yaml`: act 1 runs on real conditions (a resident, a facility, a road walked, the Hollow Den cleared, with the den forced in as a story checkpoint), acts 2 and 3 complete on flags the story sets (`main_act2_done`, `main_act3_done`), and each act's rewards start the next.
- **Ascension (built):** `content/ascension.yaml` says what must hold (default: act 3 done, faith level 5, five residents); the shrine then offers **Ascend**. The epilogue script plays with `{title}` filled in, and the ending screen names you and, for the first and only time, shows the five axes (§3): the strongest gives the epithet, it and the runner-up (if at least half as strong) the domains, God or Goddess by your form.
