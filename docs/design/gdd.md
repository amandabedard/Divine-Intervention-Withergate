...
- **Journal UI (built):** `J` in the world or Journal at the desk lists active quests with the current objective and the notes of finished stages, then finished and failed quests. A quest whose giver the corruption took is greyed and waits rather than failing, since characters can be brought back (decided K2; the resurrection itself is L1).
